...
const Students = () => {
  ...
  useEffect(() => {
    axios
      .get(process.env.NEXT_PUBLIC_URL + '/admin/users/students', {
        params: { batch },
      })
      .then((response) => {
              ...
              <tr>
                ...
                {fields.map((heading, index) =>
                  heading !== 'isActive' ? (
                    ...
                    <th
                      className="px-5 py-3 text-gray-600 text-left text-xs font-semibold uppercase flex"
                      key={index}
                    >
                      <span className="flex mr-2">ISACTIVE</span>
                      <input
                        type="checkbox"
                        className="group-hover:bg-sky-50 outline-none flex"
              ...
              {data.map((row, ridx) => (
                ...
                >
                  ...
                  {fields.map((key, kidx) =>
                    key === 'isActive' ? (
                      <td
                        className="px-6 py-4 text-sm text-gray-800 whitespace-nowrap flex justify-center align-middle"
                        key={kidx}
                      >
  ...
  return data ? (
    <>
      ...
      <br />
      <CustomTable
        data={data.filter((doc) => filterCheck(doc))}
        update={setEditedDoc}
        omit={omit}
      />
      <br />
