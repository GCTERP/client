...
const Switch = ({ initial, toggle, editable = true }) => {
    ...
    return (
        <div className="relative h-8 w-28 rounded-full text-gray-500 flex border">
            <div onClick={() => { editable && toggle(!isTrue); editable && setIsTrue(!isTrue) }} className={`absolute ${isTrue && "right-0"} h-full shadow bg-white z-10 w-7/12 border-${isTrue ? "l" : "r"} cursor-pointer rounded-full`}></div>
